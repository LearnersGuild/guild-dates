...
var isaSessionStartDates = exports.isaSessionStartDates = function isaSessionStartDates(startDate) {
  return Array.from(Array(5).keys()).map(function (sessionIdx) {
    return isaSessionStartDate(startDate, sessionIdx);
  });
};
...
function _nextMonday(date) {
  ...
  return (0, _util.momentDayOnly)(result.endOf('isoWeek').add(1, 'day'));
}