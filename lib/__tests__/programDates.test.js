...
(0, _tape2.default)('src/programDates', function (t) {
  ...
  t.test('openDaysBetween', function (tt) {
    ...
    tt.test('returns the days the guild is open between two dates', function (ttt) {
      ttt.plan(1);
  ...
  t.test('isaSessionEndDate', function (tt) {
    ...
    tt.test('returns the end date of the session (0 indexed) for a given program start date', function (ttt) {
      ttt.plan(3);
      ttt.true((0, _util.momentDayOnly)((0, _programDates.isaSessionEndDate)(new Date('2016-11-28'), 0)).isSame('2017-01-27'), 'should be Friday of 1st week, not counting break weeks');
  ...
  t.test('numDaysPerISASession', function (tt) {
    tt.test('throws if the given start date is not a date', (0, _util.throwsIfInvalidDate)(_programDates.numDaysPerISASession));

    tt.test('returns the number of days in the session (0 indexed) for a given program start date', function (ttt) {
      ttt.plan(1);
      var numDays = (0, _programDates.numDaysPerISASession)(new Date('2016-10-03'), 0);
