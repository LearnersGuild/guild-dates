...
(0, _tape2.default)('src/programDates', function (t) {
  ...
  t.test('isaSessionStartDates', function (tt) {
    tt.test('throws if the given start date is not a date', (0, _util.throwsIfInvalidDate)(_programDates.isaSessionStartDates));

    tt.test('returns the 5 session start dates for a given program start date', function (ttt) {
      ttt.plan(6);
      var ssDates = (0, _programDates.isaSessionStartDates)(new Date('2016-11-28'));
      ttt.equal(ssDates.length, 5, 'should have 5 elements');
      var expectedDateStrs = ['2016-11-28', '2017-01-30', '2017-03-27', '2017-05-22', '2017-07-24'];
      expectedDateStrs.forEach(function (dateStr, idx) {
        ttt.true((0, _util.momentDayOnly)(ssDates[idx]).isSame((0, _util.momentDayOnly)(dateStr)), 'should be ' + dateStr);
      });
    });
  });
