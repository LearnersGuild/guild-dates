...
var memorialDay = exports.memorialDay = function memorialDay(date) {
  ...
  var memDay = (0, _util.momentDayOnly)(_holidayForNthWeekdayOccurence(5, 1, 1, date));
...
var _holidaysAfter = function _holidaysAfter(date) {
  ...
  var start = (0, _util.momentDayOnly)(date);
  var specialCases = start.year() === 2017 ? [(0, _util.momentDayOnly)('2017-07-03').toDate()] : [];

  return specialCases.concat([newYearsDay(date), mlkJrDay(date), presidentsDay(date), cesarChavezDay(date), memorialDay(date), independenceDay(date), laborDay(date), indigenousPeoplesDay(date), veteransDay(date), thanksgivingDay(date), thanksgivingFriday(date), christmasEve(date), christmasDay(date)]).map(function (d) {
